...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/28 17:21:53 by hiennguy          #+#    #+#             */
/*   Updated: 2025/08/28 17:21:55 by hiennguy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_light(char **tokens, t_scene *scene)
{
	...
	if (check_obj_args_count(tokens, "Light", 3, 4) == FAIL)
		return (FAIL);
	obj = create_objects(scene, LIGHT);
	if (!obj)
		return (FAIL);
	obj->light.color = (t_color){{1.0, 1.0, 1.0}};
	obj->light.origin = parse_vec3(tokens[1], &scene->is_valid);
	if (!scene->is_valid)
		return (print_error("Light: " ERROR_COORD, tokens[1]));
	obj->light.bright_ratio = parse_float(tokens[2], &scene->is_valid);
	if (!scene->is_valid)
	...
	scene->lights_count++;
	obj->light.color = v3_mul_f32(obj->light.color, obj->light.bright_ratio);
	return (SUCCESS);
}
