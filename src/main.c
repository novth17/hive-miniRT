...
/*   Updated: 2025/07/19 00:07:48 by hiennguy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	render(t_mini_rt *minirt)
{
float	aspect_ratio = WINDOW_WIDTH / WINDOW_HEIGHT;
	...
	while (y < minirt->image->height)
	{
		...
		{
			t_v3 pixel_center = v3_add_v3(v3_add_v3(pixel00_location, f32_mul_v3((float)x, pixel_delta_u)),  f32_mul_v3((float)y, pixel_delta_v));
			...
			if (discrimanant >= 0)
			{
				...
				mlx_put_pixel(minirt->image, x, y, 0xFF0000FF);
			}
			mlx_put_pixel(minirt->image, x, y, 0xFFFFFFFF);
			++x;
		}
		++y;
	}
